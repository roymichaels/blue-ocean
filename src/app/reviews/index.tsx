...
export default function ReviewsScreen() {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      const productReviews = [
        ...reviews.filter((r) => r.productId === firstItem.product.id),
        review,
      ];
      const avgRating =
  ...
  const deleteReview = async (reviewId: string) => {
    try {
      ...
      // Update product rating and reviews count
      const productReviews = updatedReviews.filter(
        (r) => r.productId === reviewToDelete.productId
      );
      const avgRating =
      ...
      <View style={styles.starsContainer}>
        {[1, 2, 3, 4, 5].map((star) => (
          <TouchableOpacity
            ...
            onPress={() =>
              interactive && setNewReview((prev) => ({ ...prev, rating: star }))
            }
          >
            <Star
              size={size}
      ...
      >
        <View style={styles.userInfo}>
          ...
          <View>
            ...
            {item.verified && (
              <Text style={[styles.verifiedBadge, { color: colors.gold }]}>
                ✓ רכישה מאומתת
              </Text>
            )}
          </View>
        ...
        {item.comment && (
          <Text
            style={[styles.reviewComment, { color: colors.text.secondary }]}
          >
            {item.comment}
          </Text>
        )}
        ...
        <View style={styles.reviewActions}>
          ...
          >
            ...
            />
            <Text
              style={[styles.helpfulText, { color: colors.text.secondary }]}
            >
              מועיל ({item.helpful})
            </Text>
          </TouchableOpacity>
    ...
    return (
      ...
      >
        <View style={styles.orderSelectorInfo}>
          <Text
            style={[styles.orderSelectorTitle, { color: colors.text.primary }]}
          >
            הזמנה #{item.id.slice(-6)}
          </Text>
          <Text
            style={[styles.orderSelectorDate, { color: colors.text.secondary }]}
          >
            {new Date(item.createdAt).toLocaleDateString('he-IL')}
          </Text>
          <View
            style={[
              styles.orderSelectorItems,
          ...
          >
            ...
            {item.items.length > 2 && (
              <Text
                style={[styles.orderSelectorMoreItems, { color: colors.gold }]}
              >
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          style={[
            styles.tab,
            activeTab === 'my-reviews' && { backgroundColor: colors.gold },
          ]}
        ...
        >
          <Text
            style={[
              styles.tabText,
              { color: colors.text.primary },
              activeTab === 'my-reviews' && { color: colors.text.inverse },
            ]}
          >
            הביקורות שלי
          </Text>
      ...
      <View style={styles.ratingFilterContainer}>
        <Text
          style={[styles.ratingFilterLabel, { color: colors.text.primary }]}
        >
          סינון לפי דירוג:
        </Text>
        <ScrollView
            ...
            >
              <View style={styles.ratingFilterContent}>
                <Star
                  size={14}
                  color={
                    filterRating === rating ? colors.text.inverse : colors.gold
                  }
                  fill={
                    filterRating === rating ? colors.text.inverse : colors.gold
                  }
            ...
            <ScrollView style={styles.reviewForm}>
                  ...
                  {selectedOrder.items.map((item, index) => (
                    <View key={index} style={styles.selectedOrderItem}>
                      ...
                      <View style={styles.selectedOrderItemDetails}>
                        ...
                        <Text
                          style={[
                            styles.selectedOrderItemQuantity,
                            { color: colors.text.secondary },
                          ]}
                        >
                          כמות: {item.quantity}
              ...
              <TouchableOpacity
                style={[
                  styles.submitButton,
                  { backgroundColor: colors.gold },
                  (!newReview.rating || !newReview.title.trim()) && {
                    backgroundColor: colors.interactive.disabled,
                  },
                ]}
                onPress={submitReview}
                disabled={
                  !newReview.rating || !newReview.title.trim() || loading
                }
              >
                {loading ? (
