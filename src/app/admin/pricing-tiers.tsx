  ...
  const validateRules = (
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      if (r.maxQty < r.minQty)
        return 'ערך מקסימום חייב להיות גדול או שווה למינימום';
      if (r.pricePerBaseUnit == null && r.discountPct == null)
        return 'יש להזין מחיר או הנחה לכל כלל';
      if (i > 0) {
        const prev = sorted[i - 1];
        if (r.minQty !== prev.maxQty + 1)
          return 'טווחי הכמויות חופפים או מכילים חוסרים';
      }
    }
    return null;
  };
  ...
  const saveTier = async () => {
    ...
    try {
      ...
      setInfoModal({
        ...
        title: 'שגיאה',
        message:
          error instanceof Error ? error.message : 'שמירת מדרג המחירים נכשלה',
        type: 'error',
      });
    } finally {
      setLoading(false);
  ...
  const deleteTier = async (tierId: string) => {
    ...
    try {
      ...
      setInfoModal({
        ...
        title: 'שגיאה',
        message:
          error instanceof Error ? error.message : 'מחיקת מדרג המחירים נכשלה',
        type: 'error',
      });
    } finally {
      setLoading(false);
          ...
          <ScrollView style={styles.modalContent}>
              ...
              <View key={idx} style={styles.formRow}>
                ...
                <View style={styles.formGroupHalf}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      const updated = [...(newTier.rules || [])];
                      updated[idx].pricePerBaseUnit = text
                        ? parseFloat(text)
                        : undefined;
                      setNewTier({ ...newTier, rules: updated });
                    }}
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  setNewTier({ ...newTier, description: text })
                }
