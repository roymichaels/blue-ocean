...
function ensureSeed() {
  ...
  try {
    // Attempt to load seed data when running without on-chain access. JSON imports are supported by Metro/Web.
...
export async function getStore(storeId: string, id: string): Promise<Store | null> {
  ...
  if (DISABLED) {
    // Provide a sensible fallback when contracts are disabled
