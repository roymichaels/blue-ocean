...
1. **Override NEAR RPC:** restart deployments using `NEAR_RPC_URL=<backup-url>` or an appropriate flag.
2. **Flush caches** with `expo start -c` to ensure new endpoints are used.
3. **Monitor** connectivity logs from agents for successful peer discovery.
