...
At runtime the app validates that required keys are present and will throw an error if any are missing. Development-only flags like `ENABLE_UNSAFE_NEAR_PRIVATE_KEY` are automatically ignored when `NODE_ENV` is set to `production`.
